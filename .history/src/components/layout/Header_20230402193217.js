    ...
    return (
        <>
            ...
            <div className="fixed top-3 right-3 z-50">
                <div
                    className={`relative w-20 h-9 flex items-center cursor-pointer rounded-full bg-primary`}
                    onClick={() => setDark(!dark)}
                >
                    <div
                        className={`absolute transition-all duration-300 w-8 h-8 rounded-full bg-white ${
                            dark ? "left-full -translate-x-full" : "left-0"
                        }`}
                    ></div>
                </div>
            </div>
