...
function MovieMeta(type = "/videos", abc) {
    const { slug } = useParams();
    const { data } = useSWR(getMovieList(slug, type), fetcher);
    if (!data) return null;
    const { abc } = data;
    if (!abc || abc.length <= 0) return null;
    if (type === "/credits")
        return (
            <div className="mb-20">
...
function MovieCredits() {
    const { slug } = useParams();
    const { data } = useSWR(getMovieList(slug, "/credits"), fetcher);
    if (!data) return null;
    const { cast } = data;
...
function MovieVideo() {
    ...
    const { slug } = useParams();
    const { data } = useSWR(getMovieList(slug, "/videos"), fetcher);
    if (!data) return null;
    const { results } = data;
...
function MovieSimilar() {
    ...
    const { slug } = useParams();
    const { data } = useSWR(getMovieList(slug, "/similar"), fetcher);
    if (!data) return null;
    const { results } = data;
    if (!results || results.length <= 0) return null;
    return (
